...
//    TI_CC_SPIWriteReg(TI_CCxxx0_VERSION,  0x14);
//    TI_CC_SPIWriteReg(TI_CCxxx0_RSSI,    0x80);
//    TI_CC_SPIWriteReg(TI_CCxxx0_MARCSTATE,    0x01);
//    TI_CC_SPIWriteReg(TI_CCxxx0_VCO_VC_DAC,   0x94);
//}
...
void writeRFSettings(void)
{
    ...
    TI_CC_SPIWriteReg(TI_CCxxx0_PKTCTRL1, 0x05); // Packet automation control.
     //TI_CC_SPIWriteReg(TI_CCxxx0_PKTCTRL1, 0x07); // addr filter crc autoflush
    TI_CC_SPIWriteReg(TI_CCxxx0_PKTCTRL0, 0x05); // Packet automation control.
    ...
    //////////////////TI_CC_SPIWriteReg(TI_CCxxx0_MCSM1 ,   0x0F); //MainRadio Cntrl State Machine 0x3f 设置进入状态机就卡在等待GDO0上，取消设置，发送完手动进入接受状态即可
    TI_CC_SPIWriteReg(TI_CCxxx0_MCSM0 ,   0x18); //MainRadio Cntrl State Machine
    TI_CC_SPIWriteReg(TI_CCxxx0_FOCCFG,   0x1D); // Freq Offset Compens. Config
    TI_CC_SPIWriteReg(TI_CCxxx0_BSCFG,    0x1C); //  Bit synchronization config.
    TI_CC_SPIWriteReg(TI_CCxxx0_AGCCTRL2, 0xC7); // AGC control.
...
void RFSendPacket(char *txBuffer, char size)
{
  TI_CC_GDO0_PxIE &= ~TI_CC_GDO0_PIN;        // disable GDO0 interrupt to send data
  ...
  while (TI_CC_GDO0_PxIN&TI_CC_GDO0_PIN);
  TI_CC_SPIStrobe(TI_CCxxx0_SFTX);          //flush tx buffer
  TI_CC_SPIStrobe(TI_CCxxx0_SRX);
  TI_CC_GDO0_PxIFG &= ~TI_CC_GDO0_PIN;      // After pkt TX, this flag is set.
                                            // Has to be cleared before existing
  TI_CC_GDO0_PxIE |= TI_CC_GDO0_PIN;        //reinable GDO0 interrupt
}
